...
"s1.subs.done":{ko:(d,g)=>`${d} ${g} 돌파`, en:(d,g)=>`Passed ${g} on ${d}`, ja:(d,g)=>`${d} に${g}突破`},
...
"s4.foot":     {ko:n=>`티저 · Performance ver. · 일본어판 · Special Video 를 뺀 <b>본편 ${n}편</b>이다. 해당 영상을 클릭하면 해당 유튜브 페이지로 이동한다. 썸네일은 유튜브에서 그대로 불러온다.`,
                en:n=>`<b>${n} main videos</b>, excluding teasers, performance versions, Japanese editions and special videos. Click a video to open its page on YouTube. Thumbnails are loaded from YouTube.`,
                ja:n=>`ティーザー・Performance ver.・日本語版・Special Video を除いた<b>本編 ${n}本</b>です。動画をクリックするとそのYouTubeページへ移動します。サムネイルはYouTubeから読み込んでいます。`},
...
"sv.foot":     {ko:(d,how,all,keep,perkind,miss)=>`해당 영상을 클릭하면 해당 유튜브 페이지로 이동한다. 썸네일은 유튜브에서 그대로 불러온다. <b>${d}</b> 에 ${how} 으로 이 채널 전편(${all}편)을 재고, 저장소에는 조회수·좋아요 상위 ${keep}편(형식마다 ${perkind}편)만 남긴다 — 위 타일의 누적값은 남긴 편이 아니라 <b>전편</b>을 더한 값이다.${miss}`,
                en:(d,how,all,keep,perkind,miss)=>`Click a video to open its page on YouTube. Thumbnails are loaded from YouTube. On <b>${d}</b> we measured every upload on this channel (${all} of them) via ${how}, and keep only the top ${keep} by views and likes (${perkind} per format) in the repository — the totals in the tiles above are the sum of <b>all</b> uploads, not just the kept ones.${miss}`,
                ja:(d,how,all,keep,perkind,miss)=>`動画をクリックするとそのYouTubeページへ移動します。サムネイルはYouTubeから読み込んでいます。<b>${d}</b> に ${how} でこのチャンネルの全 ${all}本を測り、保存庫には再生数・高評価の上位 ${keep}本（形式ごとに ${perkind}本）だけを残しています — 上のタイルの累計は残した分ではなく<b>全本</b>の合計です。${miss}`},
...
"s5.h":        {ko:(컷,n)=>`TOP ${컷} 안에 리센느가 ${n}명`,
                en:(컷,n)=>`${n} of the TOP ${컷} are RESCENE`,
                ja:(컷,n)=>`TOP${컷}のうち${n}人がRESCENE`},
"s5.lg.res":   {ko:"리센느", en:"RESCENE", ja:"RESCENE"},
"s5.lg.other": {ko:"그 외", en:"others", ja:"その他"},
"s5.sub":      {ko:(base,idx,컷,n)=>`${base} ${idx}. 리센느는 5인조이니 TOP ${컷} 안의 리센느 ${n}명이 곧 멤버 전원이다.`,
                en:(base,idx,컷,n)=>`${base}, ${idx}. RESCENE is a five-piece, so the ${n} RESCENE members in the TOP ${컷} are the whole group.`,
                ja:(base,idx,컷,n)=>`${base} ${idx}。RESCENEは5人組なので、TOP${컷}に入る ${n}人はメンバー全員ということになります。`},
"s5.index":    {ko:null, en:"member brand reputation index", ja:"個人ブランド評判指数"},
"s5.foot":     {ko:(note,idx,man,org,cut)=>`<b>${note}</b> 지수 ${idx}를 「언급 ${man} 건」으로 읽으면 안 된다. ${cut}${org} 발표이고 원표는 brikorea 에 있다. 매월 발표라 자동으로 갱신되지 않는다.`,
                en:(note,idx,man,org,cut)=>`<b>${note}</b> An index of ${idx} must not be read as "${man} mentions". ${cut}Published by ${org}; the source table lives at brikorea. It is released monthly and does not update automatically.`,
                ja:(note,idx,man,org,cut)=>`<b>${note}</b> 指数 ${idx} を「言及 ${man}件」と読んではいけません。${cut}${org} の発表で、原表は brikorea にあります。毎月の発表のため自動更新はされません。`},
"s5.cut":      {ko:(who,rk,컷)=>`${who}가 ${rk}위라 TOP 10 에서 끊으면 멤버 한 명이 빠진다 — 그래서 ${컷}위까지 그렸다. `,
                en:(who,rk,컷)=>`${who} sits at #${rk}, so cutting at the TOP 10 would drop one member — the list runs to #${컷}. `,
                ja:(who,rk,컷)=>`${who}が${rk}位のため、TOP10で切るとメンバーが1人抜けます。だから${컷}位まで並べています。 `},
"s5.clue":     {ko:null,
...
"s8.hidden":   {ko:n=>`전체 공개가 아닌 ${n}편은 뺐다. 링크로는 열려도 채널 목록에 없는 편이다.`,
                en:n=>n===1 ? `One video is not publicly listed on the channel and is left out — it opens by link but does not appear there.` : `${n} videos are not publicly listed on the channel and are left out — they open by link but do not appear there.`,
                ja:n=>`全体公開ではない ${n}本は除いています。リンクでは開きますが、チャンネルの一覧には出ません。`},
"s8.notyet":   {ko:n=>`아직 안 올린 ${n}편은 여기 넣지 않았다.`,
                en:n=>n===1 ? `One video that is not published yet is not listed here.` : `${n} videos that are not published yet are not listed here.`,
                ja:n=>`まだ公開していない ${n}本はここに入れていません。`},
"s8.kornote":  {ko:null,
