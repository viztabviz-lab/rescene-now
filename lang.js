...
"s11.t3":      {ko:"2위는", en:"Runner-up", ja:"2位は"},
"s11.t3v":     {ko:n=>`${n}종류`, en:n=>`${n} sets`, ja:n=>`${n}種類`},
"s11.t3d":     {ko:"탭마다 다른 이모티콘", en:"a different set in each tab", ja:"タブごとに違う絵文字"},
...
"s11.rank.th3":{ko:"그 탭 2위", en:"#2 in that tab", ja:"そのタブの2位"},
...
"s7.tally2":   {ko:n=>`팬이 되어야 나오는 검색 <b>${n}개</b> — 버블 · 포카 · 콘서트 · 팬클럽 · 배경화면 · 일정`,
                en:n=>`<b>${n}</b> searches only a fan would make — Bubble, photocards, concerts, fan club, wallpapers, schedule`,
                ja:n=>`ファンでなければ出ない検索が <b>${n}件</b> — バブル・トレカ・コンサート・ファンクラブ・壁紙・スケジュール`},
"s7.meaning":  {ko:null,
...
"s7.foot":     {ko:(ex,typo)=>`${ex} 이름 오타 ${typo}개도 목록에 그대로 두었다 — 새로 알게 된 사람이 늘었다는 증거다. 왼쪽은 값이 큰 순서, 오른쪽은 <b>늘어난 크기 순서</b>라 두 목록의 눈금이 다르다.`,
                en:(ex,typo)=>`${ex} We also left ${typo} misspellings of the name in the list — evidence that more people are hearing it for the first time. The left list is ordered by size, the right by <b>how much it grew</b>, so the two use different scales.`,
                ja:(ex,typo)=>`${ex} 名前の誤字 ${typo}件もそのまま残しています — 新しく知った人が増えた証拠です。左は値の大きい順、右は<b>伸びた大きさの順</b>なので、二つのリストは目盛りが違います。`},
"s7.kornote":  {ko:null,
...
STR["s7.excl"] = {ko:null,
  en:"Of 25 rising Google searches we removed one about a person's body and two contentious ones, leaving 22; the same two were removed from the left-hand list.",
  ja:"Googleの急上昇25件のうち、身体に関する1件と論争性のある2件を除いた22件です。左のリストからも同じ2件を除いています。"};
