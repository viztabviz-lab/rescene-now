...
"sr.h":        {ko:(지,기)=>`${기}, ${지}가 1위`,
                en:(지,기)=>`${지} leads over the ${기}`,
                ja:(지,기)=>`${기}、${지}が1位`},
"sr.sub":      {ko:(지,옛순,옛기)=>`${옛기}에는 ${옛순}위였다. 기간을 좁힐수록 ${지}가 올라온다.`,
                en:(지,옛순,옛기)=>`Over the ${옛기} it ranked ${옛순}. The shorter the window, the higher ${지} climbs.`,
                ja:(지,옛순,옛기)=>`${옛기}では${옛순}位だった。期間を狭めるほど${지}が上がってくる。`},
